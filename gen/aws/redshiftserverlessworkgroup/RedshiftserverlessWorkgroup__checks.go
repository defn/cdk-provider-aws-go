...
func (r *jsiiProxy_RedshiftserverlessWorkgroup) validatePutConfigParameterParameters(value interface{}) error {
	...
	case *[]*RedshiftserverlessWorkgroupConfigParameter:
		value := value.(*[]*RedshiftserverlessWorkgroupConfigParameter)
	...
	case []*RedshiftserverlessWorkgroupConfigParameter:
		value_ := value.([]*RedshiftserverlessWorkgroupConfigParameter)
		value := &value_
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(value) {
			return fmt.Errorf("parameter value must be one of the allowed types: cdktf.IResolvable, *[]*RedshiftserverlessWorkgroupConfigParameter; received %#v (a %T)", value, value)
		}
	}
